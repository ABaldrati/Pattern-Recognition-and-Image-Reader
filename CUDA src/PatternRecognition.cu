...
void findPatternSharedMemory_wrapper(const float* __restrict__ device_target_matrix, const float* __restrict__ device_query_matrix,
	...
	switch (TILE_WIDTH) {
	case 8: {
		dim3 dimGrid(ceil(((float) target_matrix_rows) / TILE_WIDTH_S1), ceil(((float) target_matrix_columns) / TILE_WIDTH_S1));
	...
	case 16: {
		dim3 dimGrid(ceil(((float) target_matrix_rows) / TILE_WIDTH_S2), ceil(((float) target_matrix_columns) / TILE_WIDTH_S2));
	...
	case 24: {
		dim3 dimGrid(ceil(((float) target_matrix_rows) / TILE_WIDTH_S3), ceil(((float) target_matrix_columns) / TILE_WIDTH_S3));
	...
	case 32: {
		dim3 dimGrid(ceil(((float) target_matrix_rows) / TILE_WIDTH_S4), ceil(((float) target_matrix_columns) / TILE_WIDTH_S4));
...
void findPatternGlobalMemory_wrapper(const float* __restrict__ device_target_matrix, const float* __restrict__ device_query_matrix,
	...
	dim3 dimGrid(ceil(((float) sads_matrix_rows) / TILE_WIDTH), ceil(((float) sads_matrix_columns) / TILE_WIDTH));
